...
export default class roomService {
  static async create(roomDTO) {
    try {
      const existChatRoom = await RoomModel.findWithDeleted({ user: roomDTO.user });
      let roomRecord;
      if (existChatRoom.length > 0) {
        await RoomModel.restore({ user: roomDTO.user });
        const findRoom = await this.findById(roomDTO.user, 'user', existChatRoom[0]._id.toString());
        roomRecord = findRoom.body.room;
      } else {
